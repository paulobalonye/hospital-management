    ...
    public function trial_balance_report($FromDate,$ToDate,$WithOpening){
     ...
     //al hassan working
      public  function get_vouchar(){
         ...
         $date=date('Y-m-d');
          $sql="SELECT VNo, Vtype,VDate, Narration, SUM(Debit+Credit)/2 as Amount FROM acc_transaction  WHERE VDate='$date' AND VType IN ('DV','JV','CV') GROUP BY VNO, Vtype, VDate ORDER BY VDate";
